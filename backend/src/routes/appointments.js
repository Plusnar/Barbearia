...
const releaseAppointmentLocks = (appointmentId) => {
  db.query('DELETE FROM appointment_slot_locks WHERE appointment_id = ?', [appointmentId], (err) => {
    if (err) {
      if (err.code === 'ER_NO_SUCH_TABLE') return;
      console.error('Failed to release appointment locks:', err.message);
    }
  });
};
  ...
  db.getConnection(async (connectionError, connection) => {
    ...
    try {
      ...
      const serviceRows = await queryAsync(
        connection,
        'SELECT id, name, duration FROM services WHERE id = ? AND active = 1',
        [serviceId]
      );
      ...
      const barberRows = await queryAsync(
        connection,
        'SELECT id FROM users WHERE id = ? AND role = ? AND available = 1',
        [barberId, 'BARBER']
      );
...
router.put('/:id/status', (req, res) => {
  ...
  db.query(updateSql, params, (updateError, result) => {
    ...
    db.query(
      'SELECT a.id, a.customer_id, a.barber_id, a.service_id, c.name as customer_name, c.email as customer_email, c.phone as customer_phone, b.name as barber_name, s.name as service_name, s.duration, a.appointment_date, a.appointment_time, a.status, a.created_at FROM appointments a JOIN users b ON a.barber_id = b.id JOIN users c ON a.customer_id = c.id JOIN services s ON a.service_id = s.id WHERE a.id = ?',
      [id],
