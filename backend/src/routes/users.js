...
import { sendPasswordChangedNotification } from '../utils/notifications.js';
...
router.put('/password', (req, res) => {
  ...
  db.query('SELECT id, name, email, phone, password FROM users WHERE id = ?', [userId], (selectError, results) => {
    if (selectError) {
      return res.status(500).json({ success: false, message: 'Database error' });
    }
    ...
    db.query('UPDATE users SET password = ? WHERE id = ?', [hashedPassword, userId], (updateError) => {
      ...
      sendPasswordChangedNotification(user).catch((notificationError) => {
