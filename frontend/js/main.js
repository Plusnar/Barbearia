...
import { setupPasswordToggles, showRecoveryStep, startApp, switchAuth } from './auth/auth-forms.js';
...
welcomeScreen?.addEventListener('keydown', (event) => {
  if (event.key === 'Enter' || event.key === ' ') startApp(event);
});
...
$('recoveryForm').addEventListener('submit', async (event) => {
  ...
  if (!email) {
    setStatus($('authStatus'), 'Informe o e-mail da conta.', 'error');
    return;
  }

  setStatus($('authStatus'), 'Enviando codigo...');
  try {
    ...
    showRecoveryStep(2);
    setStatus($('authStatus'), 'Codigo enviado. Verifique seu e-mail (e a pasta spam).', 'ok');
    showToast('Codigo enviado', `Se ${email} estiver cadastrado, voce recebera um codigo de 6 digitos.`);
    $('resetCode')?.focus();
    $('resetPasswordForm')?.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
  } catch (error) {
    setStatus($('authStatus'), error.message, 'error');
    showToast('Falha na recuperacao', error.message, 'error');
  }
});
