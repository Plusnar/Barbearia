...
export function updateProfitPreview() {
  ...
  const barberShare = amount * (commission / 100);

  $('profitPreviewGross').textContent = money(amount);
  $('profitPreviewBarber').textContent = money(barberShare);
  $('profitPreviewHouse').textContent = money(amount - barberShare);
}
...
function calculateProfitTotals(entries) {
  return entries.reduce((totals, entry) => {
    ...
    totals.gross += amount;
    totals.barber += barberShare;
    totals.house += amount - barberShare;
    totals.entries += 1;
    return totals;
...
function renderProfitSplit(entries) {
  ...
  entries.forEach((entry) => {
    ...
    current.barberShare += barberShare;
    current.houseShare += amount - barberShare;
    current.services += 1;
    current.commissionSum += commission;
